...
class Game_Player():
	def __init__(self,game_settings,screen):
		...
		self.image = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/PR/player.png')
	...
	def update_die(self):
		...
		for self.i in range(1,3):
			self.names['player_image%s' %self.i] = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/PR/death%s.png' %self.i)
	...
	def update_image_moving_direction(self,direction):#循环图片
		if self.player_shooting == True:
			...
			for self.i in range(1,4):
				self.names['player_image%s' %self.i] = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/'+direction+'/shooting%s.png' %self.i)
				self.players.append(self.names['player_image%s' %self.i])
			...
			for self.i in range(1,6):
				self.names['player_image%s' %self.i] = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/'+direction+'/player%s.png' %self.i)
				self.players.append(self.names['player_image%s' %self.i])
		...
		if self.game_settings.player_is_alive:
			if self.player_jump == True:
				...
				for self.j in range(1,5):
					self.names1['player_image%s' %self.j] = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/'+direction+'/jump%s.png' %self.j)
	...
	def get_player_state(self,player_state):#检测player的状态是否为趴下，向上，跳跃等
		if self.player_direction == 1:
			self.image = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/PR/'+player_state+'.png')
		if self.player_direction == -1:
			self.image = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/PL/'+player_state+'.png')
		self.rect = self.image.get_rect()
		self.rect.centerx = self.center
	...
	def get_player_jump(self):
		...
		if self.player_Y > self.player_start_Y:
			...
			if self.player_direction == 1:
				...
				self.reset_player()
			if self.player_direction == -1:
				self.image = pygame.image.load('/home/renziqiang/文档/matlab/15 魂斗罗/15 ╗Ω╢╖┬▐/═╝╞¼/PL/player.png')
				self.reset_player()
